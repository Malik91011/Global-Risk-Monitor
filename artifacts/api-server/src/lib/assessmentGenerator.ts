import type { Incident, ThreatAssessment } from "@workspace/db";
...
const OPERATIONAL_GUIDANCE: Record<string, string[]> = {
  Security: [
...
export function generateThreatAssessment(
  ...
  region: string | null,
  incidents: Incident[]
): Omit<ThreatAssessment, "id" | "assessedAt"> {
  ...
  for (const incident of incidents) {
    riskCounts[incident.riskLevel as RiskLevel]++;
  ...
  else if (riskCounts.High > 0 || riskCounts.HighImpact > 2) overallRisk = "High";
  else if (riskCounts.HighImpact > 0 || riskCounts.Ongoing > 1) overallRisk = "HighImpact";
  else if (riskCounts.Ongoing > 0 || riskCounts.Moderate > 2) overallRisk = "Ongoing";
  ...
  if (region) affectedAreas.push(region);
  
  const cities = [...new Set(incidents.map(i => i.city).filter(Boolean))] as string[];
...
export function generateReportContent(
  ...
  for (const incident of incidents.slice(0, 50)) {
    ...
    if (incident.aiSummary) content += `AI Analysis: ${incident.aiSummary}\n`;
    content += "\n";
  }
