...
router.post("/generate", async (req, res) => {
    try {
        ...
        if (region)
            conditions.push(eq(incidentsTable.region, region));
        if (dateFrom)
        ...
        if (categories && categories.length > 0)
            conditions.push(inArray(incidentsTable.category, categories));
        ...
        const incidents = await db.select()
            ...
            .limit(100);
        const { content, executiveSummary, advisory, criticalCount, highCount, moderateCount, lowCount } = generateReportContent(title, country, region, incidents, includeAssessment, includeAdvisory);
