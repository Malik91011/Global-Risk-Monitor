...
router.get("/dashboard", async (_req, res) => {
    try {
        ...
        const [totalResult, criticalResult, ongoingResult, last24hResult, last7dResult, categoryResult, riskResult, recentIncidents, criticalAlerts, countriesResult,] = await Promise.all([
...
router.get("/countries", async (_req, res) => {
    try {
        const countryData = await db.select({
            ...
            count: count(),
            latitude: sql `AVG(${incidentsTable.latitude})`.as("latitude"),
            longitude: sql `AVG(${incidentsTable.longitude})`.as("longitude"),
        })
            .from(incidentsTable)
        ...
        const countries = await Promise.all(countryData.map(async (row) => {
            const [criticalResult, highResult, categoryResult] = await Promise.all([
                db.select({ count: count() }).from(incidentsTable)
                    .where(eq(incidentsTable.country, row.country) && eq(incidentsTable.riskLevel, "Critical")),
                db.select({ count: count() }).from(incidentsTable)
                    .where(eq(incidentsTable.country, row.country) && eq(incidentsTable.riskLevel, "High")),
                db.select({
                    category: incidentsTable.category,
                    count: count(),
                }).from(incidentsTable)
...
router.get("/trending", async (_req, res) => {
    try {
        ...
        const regions = await Promise.all(recentData.map(async (row) => {
            ...
            const riskLevel = (riskResult[0]?.riskLevel || "Low");
            return {
                name,
                country: row.country,
